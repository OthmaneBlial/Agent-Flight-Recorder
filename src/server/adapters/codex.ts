        ...
        if (event.kind === 'file' && event.path) {
          ...
          for (const path of eventPaths(event)) {
            if (start > 0) {
              ...
              if (phase === 'before')
                store.insertCaptureGap(
                  event,
                  'best_effort_pre_state',
                  'Codex was observed through its append-only log; the captured pre-state may already include the completed mutation.',
                  path,
                );
...
function mapCodexRecord(record: JsonRecord, sessionId: string, sequence: number, calls: Map<string, CallDescriptor>): EventInput[] {
  const payload = object(record.payload);
    ...
    return [
      {
        ...
        summary: `Codex ${string(payload.cli_version) ?? 'unknown version'} · ${string(payload.cwd) ?? 'unknown workspace'}`,
        status: 'success',
        actor: 'codex',
        payload,
      },
    ...
    return [
      {
        ...
        id: id(),
        kind: 'context',
        title: 'Workspace state captured',
        summary: payload.full === true ? 'Full workspace state snapshot' : 'Workspace state delta',
        status: 'neutral',
        actor: 'runtime',
        payload,
      },
    ...
    return [
      {
        ...
        kind,
        title,
        summary: summarize(content) || 'Empty message',
        status: 'success',
        actor: role,
        payload: { role, phase: payload.phase ?? null, content },
      },
    ];
  }
    ...
    return [
      {
        ...
        summary: summary || `Encrypted reasoning payload · ${formatBytes(encryptedLength)}`,
        status: 'success',
        actor: 'assistant',
        payload: { summary, encryptedBytes: encryptedLength },
      },
    ];
  }
    ...
    return [
      {
        ...
        summary: summarize(message) || 'Empty prompt',
        status: 'success',
        actor: 'user',
        payload: { message, images: payload.images ?? [], localImages: payload.local_images ?? [], textElements: payload.text_elements ?? [] },
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'response',
        title: phase === 'commentary' ? 'Agent progress update' : 'Agent response',
        summary: summarize(message) || 'Empty response',
        status: 'success',
        actor: 'assistant',
        payload: { message, phase, memoryCitation: payload.memory_citation ?? null },
      },
    ];
  }
    ...
    return [
      {
        ...base,
        id: id(),
        kind: eventKindForTool('exec_command', input),
        title: `${toolTitle(eventKindForTool('exec_command', input), ['exec_command'])} result`,
        summary: summarize(nativeOutput(payload)) || `Command ${status}`,
        status,
        actor: 'runtime',
        callId: string(payload.call_id),
        command,
        durationMs: nativeDurationMs(payload.duration),
        payload: {
          phase: 'result',
          tool: 'exec_command',
          input,
          output: nativeOutput(payload),
          exitCode: finiteNumber(payload.exit_code),
    ...
    return [
      {
        ...
        id: id(),
        kind: 'file',
        title: 'File mutation result · apply_patch',
        summary: summarize(paths.join(', ') || nativeOutput(payload)) || `Patch ${status}`,
        status,
        actor: 'runtime',
        callId: string(payload.call_id),
        path: paths[0] ?? null,
    ...
    return [
      {
        ...
        id: id(),
        kind: 'tool',
        title: 'Tool invocation · view_image',
        summary: path ?? 'Image inspection',
        status: 'running',
        actor: 'assistant',
        callId: string(payload.call_id),
        path,
        payload: { phase: 'call', tool: 'view_image', input: { path }, paths: path ? [path] : [] },
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'tool',
        title: 'Tool invocation · web_search',
        summary: summarize(JSON.stringify(input)),
        status: 'running',
        actor: 'assistant',
        payload: { phase: 'call', tool: 'web_search', input },
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'tool',
        title: 'Tool invocation · web_search result',
        summary: summarize(JSON.stringify(input)),
        status: 'success',
        actor: 'runtime',
        callId: string(payload.call_id),
        payload: { phase: 'result', tool: 'web_search', input, output: { query: payload.query ?? null } },
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'tool',
        title: 'Tool invocation · tool_search',
        summary: summarize(JSON.stringify(input)),
        status: 'running',
        actor: 'assistant',
        callId: string(payload.call_id),
        payload: { phase: 'call', tool: 'tool_search', input, execution: payload.execution ?? null },
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'tool',
        title: 'Tool invocation · tool_search result',
        summary: `${Array.isArray(payload.tools) ? payload.tools.length : 0} tools returned`,
        status: string(payload.status) === 'failed' ? 'error' : 'success',
        actor: 'runtime',
        callId: string(payload.call_id),
        payload: { phase: 'result', tool: 'tool_search', input, output: payload.tools ?? [], execution: payload.execution ?? null },
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'tool',
        title: `Tool invocation · ${tool} result`,
        summary: summarize(outputText(result)),
        status: inferResultStatus(result),
        actor: 'runtime',
        callId: string(payload.call_id),
        durationMs: nativeDurationMs(payload.duration),
        payload: { phase: 'result', tool, input, output: result, connectorId: payload.connector_id ?? null },
      },
    ];
  }
    ...
    return [
      {
        ...
        summary: `Turn ${string(payload.turn_id)?.slice(0, 8) ?? 'unknown'} · ${formatNumber(finiteNumber(payload.model_context_window) ?? 0)} token context`,
        status: 'running',
        actor: 'runtime',
        payload,
      },
    ...
    return [
      {
        ...
        summary: summarize(string(payload.last_agent_message) ?? 'Turn complete'),
        status: 'success',
        actor: 'runtime',
        payload,
      },
    ...
    return [
      {
        ...
        id: id(),
        kind: 'context',
        title: 'Thread objective updated',
        summary: summarize(string(goal.objective) ?? 'Objective state changed'),
        status: 'neutral',
        actor: 'operator',
        payload,
      },
  ...
  if (type === 'event_msg' && payloadType === 'item_completed') {
      ...
      return [
        {
          ...base,
          id: id(),
          kind: isUser ? 'prompt' : 'response',
          title: isUser ? 'Operator prompt completed' : string(item.phase) === 'commentary' ? 'Agent progress completed' : 'Agent response completed',
          summary: summarize(content) || `Empty ${itemType}`,
          status: 'success',
    ...
    if (itemType === 'Reasoning') {
      const summary = Array.isArray(item.summary_text)
        ? (item.summary_text as unknown[]).filter((value): value is string => typeof value === 'string').join('\n')
        : '';
      return [
        {
          ...base,
          id: id(),
          kind: 'reasoning',
          title: 'Reasoning stage completed',
          summary: summarize(summary) || 'Reasoning stage completed',
          status: 'success',
          actor: 'assistant',
          durationMs: duration,
          payload: { itemType, itemId, summary },
        },
      ];
    }
      ...
      return [
        {
          ...
          id: id(),
          kind,
          title: `${toolTitle(kind, ['exec_command'])} completed`,
          summary: summarize(nativeOutput(item)) || `Command ${commandResultStatus(item)}`,
          status: commandResultStatus(item),
          actor: 'runtime',
          callId: itemId,
          command,
          durationMs: nativeDurationMs(item.duration) ?? duration,
          payload: { phase: 'result', tool: 'exec_command', input, output: nativeOutput(item), exitCode: finiteNumber(item.exit_code), runtimeItemId: itemId },
        },
      ];
    }
      ...
      return [
        {
          ...
          id: id(),
          kind: 'file',
          title: 'File mutation completed',
          summary: summarize(paths.join(', ') || nativeOutput(item)),
          status,
          actor: 'runtime',
          callId: itemId,
          path: paths[0] ?? null,
          durationMs: duration,
          payload: { phase: 'result', tool: 'apply_patch', input: { paths }, output: nativeOutput(item), paths, changes, runtimeItemId: itemId },
        },
      ];
    }
      ...
      return [
        {
          ...
          id: id(),
          kind: 'tool',
          title: 'Tool invocation · view_image result',
          summary: path ?? 'Image inspection completed',
          status: 'success',
          actor: 'runtime',
          callId: itemId,
          path,
          durationMs: duration,
          payload: { phase: 'result', tool: 'view_image', input: { path }, output: null, runtimeItemId: itemId },
        },
      ];
    }
      ...
      return [
        {
          ...
          id: id(),
          kind: 'tool',
          title: `Tool invocation · ${tool} completed`,
          summary: summarize(outputText(item.result)),
          status: inferResultStatus(item.result),
          actor: 'runtime',
          callId: itemId,
          durationMs: nativeDurationMs(item.duration) ?? duration,
          payload: { phase: 'result', tool, input, output: item.result ?? null, runtimeItemId: itemId },
        },
      ];
    }
      ...
      return [
        {
          ...
          id: id(),
          kind: 'tool',
          title: `Extension completed · ${string(item.kind) ?? 'search'}`,
          summary: summarize(string(item.query) ?? JSON.stringify(input)),
          status: 'success',
          actor: 'runtime',
          callId: itemId,
          durationMs: duration,
          payload: { phase: 'result', tool: string(item.kind) ?? 'extension', input, output: item.results ?? [], runtimeItemId: itemId },
        },
      ];
    }
      ...
      return [
        {
          ...
          summary: 'Runtime context was compacted',
          status: 'success',
          actor: 'runtime',
          durationMs: duration,
      ...
      return [
        {
          ...
          kind: 'context',
          title: 'Plan updated',
          summary: summarize(string(item.text) ?? 'Plan state changed'),
          status: 'success',
          actor: 'assistant',
          durationMs: duration,
          payload: { item },
      ...
      return [
        {
          ...
          id: id(),
          kind: 'lifecycle',
          title: 'Subagent activity completed',
          summary: `${string(item.kind) ?? 'activity'} · ${string(item.agent_thread_id) ?? 'unknown agent'}`,
          status: 'success',
          actor: 'runtime',
          callId: itemId,
          durationMs: duration,
      ...
      return [
        {
          ...
          id: id(),
          kind: 'tool',
          title: `Collaboration tool completed · ${string(item.tool) ?? 'agent tool'}`,
          summary: summarize(JSON.stringify(item.receiver_agents ?? item.receiver_thread_ids ?? [])),
          status: string(item.status) === 'failed' ? 'error' : 'success',
          actor: 'runtime',
          callId: itemId,
          durationMs: duration,
          payload: {
            ...
            input: { receiverAgents: item.receiver_agents ?? [], receiverThreadIds: item.receiver_thread_ids ?? [] },
    ...
    return [
      {
        ...
        id: id(),
        kind: 'lifecycle',
        title: 'Runtime item completed',
        summary: `${itemType} · ${duration} ms`,
        status: 'success',
        actor: 'runtime',
        durationMs: duration,
        payload: compactPayload(payload, ['started_at_ms', 'completed_at_ms', 'turn_id', 'item']),
      },
    ];
  }
    ...
    return [
      {
        ...
        id: id(),
        kind: 'error',
        title: 'Agent turn aborted',
        summary: summarize(string(payload.reason) ?? 'Turn aborted'),
        status: 'error',
        actor: 'runtime',
    ...
    return [
      {
        ...
        summary: `${finiteNumber(payload.num_turns) ?? 0} turns rolled back`,
        status: 'success',
        actor: 'runtime',
        payload,
      },
    ...
    return [
      {
        ...
        id: id(),
        kind: 'context',
        title: payloadType === 'context_compacted' ? 'Context compacted' : 'Thread settings applied',
        summary: payloadType === 'context_compacted' ? 'Runtime context was compacted' : 'Execution settings changed',
        status: 'success',
        actor: 'runtime',
        payload,
      },
    ...
    return [
      {
        ...
        id: id(),
        kind: 'context',
        title: 'Compacted history record',
        summary: 'Native history compaction boundary',
        status: 'success',
        actor: 'runtime',
        payload,
      },
    ...
    return [
      {
        ...
        summary: 'Native collaboration routing metadata',
        status: 'success',
        actor: 'runtime',
        payload,
      },
