...
import type { CallLineage, CodeEvolution, EventKind, FileSnapshot, Overview, PermissionTrace, RecorderEvent, RecordedSession, SessionComparison, SourceHealth } from '../shared/types';
...
export function App() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api.events(selectedSessionId).then((nextEvents) => {
      if (cancelled) return;
  ...
  const selectEvolutionPath = useCallback((path: string) => {
    if (!selectedEventId) return;
    api.evolution(selectedEventId, path).then(setEvolution).catch((nextError) => setError(String(nextError)));
  }, [selectedEventId]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api.compare(selectedSessionId, comparisonSessionId).then((nextComparison) => {
  ...
  useEffect(() => {
    ...
    source.addEventListener('ingest', () => {
      void loadShell();
      if (selectedRef.current) api.events(selectedRef.current).then(setEvents).catch(() => undefined);
    });
  ...
  const visibleSessions = useMemo(() => {
    ...
    return sessions.filter((session) => {
      ...
      return !needle || [flightTitle(session.title, session.projectName), session.projectName, session.cwd].some((value) => value?.toLowerCase().includes(needle));
    });
  ...
  const visibleEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      const filterMatches = activeFilter === 'all'
        || (activeFilter === 'error' ? event.status === 'error' : event.kind === activeFilter);
  ...
  const cursor = Math.max(0, visibleEvents.findIndex((event) => event.id === selectedEventId));
  ...
  useEffect(() => {
    ...
    function selectedRefEvent() { return selectedEventId; }
  }, [playing, selectedEventId, visibleEvents]);

  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if ((event.target as HTMLElement)?.matches('input, textarea, select')) return;
      if (event.key.toLowerCase() === 'j' || event.key === 'ArrowDown') { event.preventDefault(); moveCursor(1); }
      if (event.key.toLowerCase() === 'k' || event.key === 'ArrowUp') { event.preventDefault(); moveCursor(-1); }
  ...
  const scanNow = async () => {
    setScanning(true);
    try { await api.scan(); await loadShell(); }
    catch (nextError) { setError(String(nextError)); }
    ...
    <div className="recorder-shell">
      <header className="masthead">
        <div className="brand-lockup">
          <div className="brand-mark" aria-hidden="true"><span>AF</span><span>R</span></div>
          <div>
            <p className="eyebrow">LOCAL OBSERVABILITY SYSTEM / 01</p>
            <h1>AGENT FLIGHT RECORDER</h1>
          </div>
        </div>
        <div className="masthead-stats" aria-label="Recorder status">
          <span><HardDrive size={14} /> {formatCompact(overview?.events ?? 0)} EVENTS</span>
          <span><FileDiff size={14} /> {formatCompact(overview?.snapshots ?? 0)} SNAPSHOTS · {formatCompact(overview?.captureGaps ?? 0)} GAPS</span>
          <span><ShieldCheck size={14} /> AES-GCM · DB V{overview?.schemaVersion ?? '—'}</span>
          <span className={connected ? 'signal-online' : 'signal-offline'}><i /> {connected ? 'LIVE LINK' : 'RECONNECTING'}</span>
        </div>
        <button className="scan-button" onClick={() => void scanNow()} disabled={scanning}>
          <RefreshCw size={14} className={scanning ? 'spin' : ''} /> {scanning ? 'SCANNING' : 'SCAN NOW'}
        </button>
      </header>

      {error && <div className="error-banner"><AlertTriangle size={15} /> <span>{error}</span><button onClick={() => setError(null)} aria-label="Dismiss error"><X size={14} /></button></div>}

      <div className="cockpit-grid">
        <aside className="session-bay" aria-label="Recorded sessions">
          <div className="panel-heading">
            <div><span className="section-index">01</span><h2>RECORDED FLIGHTS</h2></div>
          ...
          <div className="provider-switch" aria-label="Provider filter">
            {(['all', 'codex', 'opencode', 'claude', 'cursor', 'compatible'] as const).map((value) => (
              <button key={value} className={provider === value ? 'active' : ''} onClick={() => setProvider(value)}>{value === 'compatible' ? 'compat' : value}</button>
            ))}
          </div>
          <div className="session-list">
            {visibleSessions.map((session, index) => (
              <button key={session.id} className={`session-card ${session.id === selectedSessionId ? 'selected' : ''}`} onClick={() => { setSelectedSessionId(session.id); setPlaying(false); }}>
                ...
                <span className="session-copy">
                  ...
                  <span>{session.projectName} · {relativeTime(session.updatedAt)}</span>
                </span>
          ...
          <div className="source-rack">
            ...
            {sources.map((source) => <div key={source.provider} title={source.detail}><i className={source.available ? 'available' : ''} /><b>{source.provider}</b><small>{source.detail}</small></div>)}
            {captureHealth?.delivery.components.map((component) => <div key={component.component} title={`Last receipt ${relativeTime(component.lastSeenAt)}`}><i className={component.state === 'active' ? 'available' : ''} /><b>{component.component}</b><small>{component.state === 'active' ? 'HEARTBEAT ACTIVE' : `IDLE · ${relativeTime(component.lastSeenAt)}`}</small></div>)}
            {captureHealth && <div title={captureHealth.delivery.limitation}><i className={captureHealth.status === 'healthy' ? 'available' : ''} /><b>evidence</b><small>{captureHealth.calls.stale} STALE CALLS · {captureHealth.permissions.unknown} UNKNOWN PERMISSIONS</small></div>}
          </div>
        </aside>
            ...
            <>
              <section className="flight-header">
                <div className="flight-title-row">
                  ...
                  <label className="comparison-control"><span>COMPARE TARGET</span><select value={comparisonSessionId} onChange={(event) => setComparisonSessionId(event.target.value)}><option value="">NONE</option>{sessions.filter((session) => session.id !== selectedSession.id).map((session) => <option key={session.id} value={session.id}>{flightTitle(session.title, session.projectName)} · {session.provider}</option>)}</select></label>
                  <div className="flight-agent">
                    ...
                    <div><b>{selectedSession.provider}</b><span>{selectedSession.model ?? selectedSession.agentVersion ?? 'native adapter'}</span></div>
                  </div>
                </div>
              ...
              <section className="playback-deck" aria-label="Timeline playback controls">
                <button className="transport-main" onClick={() => setPlaying((value) => !value)} aria-label={playing ? 'Pause replay' : 'Play replay'}>{playing ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}</button>
                <button className="transport-step" onClick={() => moveCursor(-1)} aria-label="Previous event"><ChevronLeft size={16} /></button>
                <button className="transport-step" onClick={() => moveCursor(1)} aria-label="Next event"><ChevronRight size={16} /></button>
                <span className="timecode">{String(cursor + 1).padStart(4, '0')} <i>/</i> {String(visibleEvents.length).padStart(4, '0')}</span>
                <input
                  className="scrubber"
                  type="range"
                  min={0}
              ...
              <section className="filter-deck">
                <div className="filter-row">
                  {FILTERS.map((filter) => {
                    const count = filter.id === 'all' ? events.length : filter.id === 'error' ? events.filter((event) => event.status === 'error').length : events.filter((event) => event.kind === filter.id).length;
                    return <button key={filter.id} className={activeFilter === filter.id ? 'active' : ''} onClick={() => setActiveFilter(filter.id)}>{filter.label}<span>{count}</span></button>;
                  })}
                </div>
                <label className="event-search"><Search size={13} /><input value={eventQuery} onChange={(event) => setEventQuery(event.target.value)} placeholder="FIND IN TIMELINE" aria-label="Search events" /></label>
              </section>

              <div className="timeline-scroll" ref={timelineRef} aria-label="Session event timeline">
                  ...
                  {eventVirtualizer.getVirtualItems().map((virtualRow) => {
                    ...
                    return (
                      <EventRow
                        ...
                        onSelect={() => { setSelectedEventId(event.id); setPlaying(false); }}
                ...
                {!visibleEvents.length && <div className="no-events"><CircleDot size={22} /><b>NO SIGNALS IN THIS CHANNEL</b><span>Adjust filters or search terms.</span></div>}
              </div>
            </>
        ...
        <aside className="inspector-bay" aria-label="Event inspector">
          <div className="panel-heading"><div><span className="section-index">03</span><h2>EVENT INSPECTOR</h2></div>{eventDetail && <span className={`status-stamp status-${eventDetail.status}`}>{eventDetail.status}</span>}</div>
          {eventDetail ? <EventInspector event={eventDetail} evolution={evolution} lineage={lineage} permissionTrace={permissionTrace} onEvolutionPath={selectEvolutionPath} /> : <div className="inspector-empty"><Zap size={28} /><p>Select a signal to inspect its native evidence.</p></div>}
        </aside>
      </div>
    </div>
  );
...
function ComparisonStrip({ comparison }: { comparison: SessionComparison }) {
  return <section className="comparison-strip" aria-label="Session comparison"><div className="comparison-caption"><span>TARGET Δ</span><b>{flightTitle(comparison.right.title, 'Recorded flight')}</b><small>relative to {flightTitle(comparison.left.title, 'recorded flight')}</small></div><SignedMetric label="EVENTS" value={comparison.metricDelta.totalEvents} /><SignedMetric label="TOOLS" value={comparison.metricDelta.toolCalls} /><SignedMetric label="TESTS" value={comparison.metricDelta.testRuns} /><SignedMetric label="FAILURES" value={comparison.metricDelta.errors} adverse /><SignedMetric label="TOKENS" value={comparison.metricDelta.tokensIn + comparison.metricDelta.tokensOut} /><SignedMetric label="GAPS" value={comparison.metricDelta.captureGaps} adverse /><div className="comparison-files"><b>{comparison.files.shared.length}</b><span>SHARED FILES</span><small>−{comparison.files.leftOnly.length} / +{comparison.files.rightOnly.length}</small></div></section>;
}
...
function SignedMetric({ label, value, adverse = false }: { label: string; value: number; adverse?: boolean }) {
  ...
  return <div className={`signed-metric ${tone}`}><b>{value > 0 ? '+' : ''}{formatCompact(value)}</b><span>{label}</span></div>;
}

function EventRow({ event, index, start, selected, onSelect, measureRef, offset }: { event: RecorderEvent; index: number; start: string; selected: boolean; onSelect: () => void; measureRef: (node: Element | null) => void; offset: number }) {
  const meta = KIND_META[event.kind];
  const Icon = meta.icon;
  return (
    <button ref={measureRef} data-index={index} style={{ transform: `translateY(${offset}px)` }} className={`event-row virtual ${selected ? 'selected' : ''} tone-${meta.tone} status-${event.status}`} onClick={onSelect}>
      ...
      <span className="event-node"><Icon size={14} strokeWidth={1.8} /></span>
      <span className="event-body">
        <span className="event-kicker"><b>{meta.label}</b>{event.durationMs !== null && <i>{duration(event.durationMs)}</i>}{event.actor && <i>{event.actor}</i>}</span>
        <strong>{event.title}</strong>
        <span>{event.summary || 'No summary emitted'}</span>
        {(event.command || event.path) && <code>{event.path ?? firstLine(event.command ?? '')}</code>}
      </span>
      <span className="event-status" aria-label={event.status} />
    </button>
  );
}

function EventInspector({ event, evolution, lineage, permissionTrace, onEvolutionPath }: { event: RecorderEvent; evolution: CodeEvolution | null; lineage: CallLineage | null; permissionTrace: PermissionTrace | null; onEvolutionPath: (path: string) => void }) {
  const meta = KIND_META[event.kind];
  const Icon = meta.icon;
  ...
  return (
    <div className="inspector-scroll">
      <section className={`inspector-hero tone-${meta.tone}`}>
        <span className="inspector-icon"><Icon size={19} /></span>
        <p>{meta.label.toUpperCase()} / #{String(event.sequence).padStart(5, '0')}</p>
        <h3>{event.title}</h3>
      ...
      {event.command && <InspectorBlock title="COMMAND"><pre>{event.command}</pre></InspectorBlock>}
      {event.path && <InspectorBlock title="AFFECTED PATH"><code className="path-code">{event.path}</code></InspectorBlock>}
      ...
      {(event.tokensIn !== null || event.tokensOut !== null) && (
        <InspectorBlock title="RESOURCE METER"><div className="resource-grid"><DataPoint label="INPUT" value={formatCompact(event.tokensIn ?? 0)} /><DataPoint label="OUTPUT" value={formatCompact(event.tokensOut ?? 0)} /><DataPoint label="CACHED" value={formatCompact(event.cachedTokens ?? 0)} /></div></InspectorBlock>
      )}
      <InspectorBlock title="NORMALIZED PAYLOAD"><JsonTree value={payload} /></InspectorBlock>
      {event.raw !== undefined && <details className="raw-evidence"><summary>RAW NATIVE EVIDENCE</summary><pre>{JSON.stringify(event.raw, null, 2)}</pre></details>}
    </div>
  );
}
  ...
  return (
    <InspectorBlock title="PERMISSION EVIDENCE">
      <div className={`permission-verdict permission-${flow.outcome}`}>
        <ShieldCheck size={15} />
        <span><b>{flow.outcome}</b><small>{flow.assurance} evidence</small></span>
      </div>
      ...
      {flow.assurance === 'inferred' && <p className="permission-caveat">Execution was observed, but the provider did not emit the operator’s manual allow/deny response.</p>}
      {flow.assurance === 'unresolved' && <p className="permission-caveat">A permission request was observed; no correlated decision or execution evidence has arrived.</p>}
    </InspectorBlock>
  );
}
    ...
    <InspectorBlock title="ATTEMPT LINEAGE">
      <div className="attempt-lineage">
        {lineage.attempts.map((attempt) => {
          ...
          return <div className={`attempt-card outcome-${attempt.outcome} ${active ? 'active' : ''}`} key={attempt.eventId}>
            <span>ATTEMPT {String(attempt.attempt).padStart(2, '0')}</span>
            <b>{attempt.outcome}</b>
            <small>{elapsed === null ? 'result pending' : duration(elapsed)} · {attempt.callId?.slice(0, 12) ?? 'no call id'} · {attempt.facets} facet{attempt.facets === 1 ? '' : 's'}</small>
...
function FileEvolution({ event, evolution, onPath }: { event: RecorderEvent; evolution: CodeEvolution | null; onPath: (path: string) => void }) {
  ...
  return (
    <InspectorBlock title="CODE EVOLUTION">
      {evolution && evolution.availablePaths.length > 1 && <div className="evolution-paths" aria-label="Affected files">{evolution.availablePaths.map((path) => <button key={path} className={path === evolution.path ? 'active' : ''} onClick={() => onPath(path)} title={path}>{path.split(/[/\\]/).pop()}</button>)}</div>}
      {evolution && (evolution.before || evolution.after) && <div className="snapshot-pair"><SnapshotCard label="BEFORE" snapshot={evolution.before} /><span className="snapshot-arrow">→</span><SnapshotCard label="AFTER" snapshot={evolution.after} /></div>}
      {evolution?.gaps.map((gap) => <div className="evolution-gap" key={gap.eventId}><AlertTriangle size={12} /><span><b>{gap.code}</b>{gap.message}</span></div>)}
...
function DiffView({ diff }: { diff: string }) {
  return <div className="diff-view" aria-label="Recorded code change">{diff.split('\n').slice(0, 1200).map((line, index) => {
    const className = line.startsWith('+') && !line.startsWith('+++') ? 'added' : line.startsWith('-') && !line.startsWith('---') ? 'removed' : line.startsWith('@@') || line.startsWith('***') ? 'marker' : '';
    return <span className={className} key={`${index}-${line.slice(0, 12)}`}>{line || ' '}</span>;
  })}</div>;
}

function SnapshotCard({ label, snapshot }: { label: string; snapshot: FileSnapshot | null }) {
  return <div className={`snapshot-card ${snapshot ? `snapshot-${snapshot.status}` : 'snapshot-unavailable'}`}><span>{label}</span><b>{snapshot?.status ?? 'unavailable'}</b><code>{snapshot?.hash?.slice(0, 10) ?? '—'}</code><small>{snapshot ? `${snapshot.assurance} · ${snapshot.byteSize === null ? '—' : formatBytes(snapshot.byteSize)}` : 'boundary missing'}</small></div>;
}
...
function InspectorBlock({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="inspector-block"><h4>{title}</h4>{children}</section>;
}

function DataPoint({ label, value, wide = false }: { label: string; value: string; wide?: boolean }) {
  return <div className={wide ? 'data-point wide' : 'data-point'}><span>{label}</span><b title={value}>{value}</b></div>;
}

function Metric({ label, value, compact = false, alert = false }: { label: string; value: number; compact?: boolean; alert?: boolean }) {
  return <div className={alert ? 'metric alert' : 'metric'}><b>{compact ? formatCompact(value) : value.toLocaleString()}</b><span>{label}</span></div>;
}

function EmptySessions({ sources }: { sources: SourceHealth[] }) {
  const detected = sources.filter((source) => source.available).length;
  return <div className="session-empty"><HardDrive size={24} /><b>NO FLIGHTS LOADED</b><span>{detected} native source{detected === 1 ? '' : 's'} detected. Run a scan to ingest local evidence.</span></div>;
}

function WelcomeState({ onScan, scanning }: { onScan: () => void; scanning: boolean }) {
  return (
    <div className="welcome-state">
      <div className="radar-mark"><span /><span /><CircleDot size={30} /></div>
      <p className="eyebrow">BLACK BOX READY</p>
      <h2>MAKE THE INVISIBLE<br />EXECUTION VISIBLE.</h2>
      <p>Native agent traces stay on this machine, normalized into an auditable timeline without cloud telemetry.</p>
      <button onClick={onScan} disabled={scanning}><RefreshCw size={15} className={scanning ? 'spin' : ''} /> {scanning ? 'SCANNING SOURCES' : 'SCAN LOCAL SOURCES'}</button>
    </div>
  );
}
