...
describe('official provider event contracts', () => {
  it('ingests all 31 documented Claude Code hook events without dropping the native envelope', () => {
    ...
    const receipts = CLAUDE_HOOK_EVENTS.map((event, index) => {
      const payload = {
        session_id: 'claude-contract', prompt_id: 'prompt-1', transcript_path: '/tmp/claude-contract.jsonl',
        cwd: '/work/claude-contract', permission_mode: 'default', effort: { level: 'high' }, hook_event_name: event,
  ...
  it('ingests all 21 documented native Cursor events, including the unscoped workspace event', () => {
    ...
    const receipts = CURSOR_HOOK_EVENTS.map((event, index) => {
      const sessionFields = event === 'workspaceOpen' ? {} : {
        conversation_id: 'cursor-contract', generation_id: 'generation-1', model: 'composer-model', model_id: 'model-1',
        model_params: [{ id: 'thinking', value: 'high' }], transcript_path: '/tmp/cursor-contract.jsonl',
      };
      const payload = {
        ...sessionFields, hook_event_name: event, cursor_version: '1.7.0', workspace_roots: ['/work/cursor-contract'],
        user_email: null, timestamp: new Date(Date.parse('2026-08-20T11:00:00.000Z') + index * 1_000).toISOString(),
        ...cursorFields(event),
      };
...
function claudeFields(event: typeof CLAUDE_HOOK_EVENTS[number]): Record<string, unknown> {
  const fields: Record<typeof CLAUDE_HOOK_EVENTS[number], Record<string, unknown>> = {
    ...
    PostToolUse: { tool_name: 'Bash', tool_input: { command: 'npm test -- contract' }, tool_use_id: 'claude-tool-1', tool_response: { output: 'passed' }, duration_ms: 4 },
    PostToolUseFailure: { tool_name: 'Bash', tool_input: { command: 'npm test -- failure' }, tool_use_id: 'claude-tool-2', error: 'failed', is_interrupt: false, duration_ms: 5 },
    PostToolBatch: { tool_calls: [{ tool_name: 'Read', tool_input: { file_path: '/work/claude-contract/a.ts' }, tool_use_id: 'claude-batch-1', tool_response: { content: 'a' } }] },
    ...
    SubagentStop: { stop_hook_active: false, agent_id: 'agent-1', agent_type: 'Explore', agent_transcript_path: '/tmp/agent-1.jsonl', last_assistant_message: 'Done', background_tasks: [], session_crons: [] },
...
function cursorFields(event: typeof CURSOR_HOOK_EVENTS[number]): Record<string, unknown> {
  const fields: Record<typeof CURSOR_HOOK_EVENTS[number], Record<string, unknown>> = {
    ...
    postToolUse: { tool_name: 'Shell', tool_input: { command: 'pnpm test -- generic' }, tool_output: JSON.stringify({ output: 'passed' }), tool_use_id: 'cursor-tool-1', cwd: '/work/cursor-contract', duration: 7 },
    postToolUseFailure: { tool_name: 'Shell', tool_input: { command: 'pnpm test -- failed' }, tool_use_id: 'cursor-tool-2', cwd: '/work/cursor-contract', error_message: 'failed', failure_type: 'error', duration: 8, is_interrupt: false },
    ...
    afterMCPExecution: { tool_name: 'database_query', tool_input: JSON.stringify({ query: 'select 1' }), result_json: JSON.stringify({ rows: [1] }), duration: 10 },
    ...
    subagentStart: { subagent_id: 'subagent-1', subagent_type: 'explore', task: 'Inspect', parent_conversation_id: 'cursor-contract', tool_call_id: 'subagent-call-1', subagent_model: 'model-1', is_parallel_worker: false },
    subagentStop: { subagent_type: 'explore', status: 'completed', task: 'Inspect', description: 'Audit', summary: 'Done', duration_ms: 12, message_count: 2, tool_call_count: 1, loop_count: 1, modified_files: [], agent_transcript_path: '/tmp/subagent.jsonl' },
    afterAgentThought: { text: 'A completed thinking block', duration_ms: 11 },
    afterAgentResponse: { text: 'A final response' },
    preCompact: { trigger: 'auto', context_usage_percent: 80, context_tokens: 80_000, context_window_size: 100_000, message_count: 30, messages_to_compact: 10, is_first_compaction: true },
