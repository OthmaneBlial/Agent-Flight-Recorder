...
Which current, officially documented local hook events and payload fields can Agent Flight Recorder use to capture Codex, Claude Code, and Cursor activity without depending on undocumented private storage or sending data to a cloud service?

## Subtopics

1. Codex hooks
   - Confirm lifecycle, prompt, tool, permission, subagent, compaction, and session events.
   - Separate the official live hook contract from the explicitly unstable transcript backfill format.
